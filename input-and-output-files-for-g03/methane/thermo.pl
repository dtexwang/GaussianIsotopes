...
#
# modified by DTW 8/30/2014 to increase precision of frequencies displayed
$pi = 3.1415926535;
$avogad = 6.022137E23;
@specialt = ( 298.15 );
if ( $#ARGV < 0 ) {
...
for ( $i = 0; $i < $ifreq; $i++ ) {
	$tt = $freq[$i];
	$freq[$i] *= $scale;
	printf( "%3d\t%6.2f\t\t%6.2f\n", $i + 1, $tt, $freq[$i] );  # edited DTW, to report 2 decimal places
...
unless ( $datfile ) {
	...
	if ( $reply =~ /^y/i ) {
		...
		if ( $ifreq > 0 ) {
			print DAT "VIB\t";
			for ( $i = 0; $i < $ifreq; $i++ ) {
				printf DAT "%.2f ", $freq[$i];	# these are scaled frequencies
			}
			print DAT "\n";
		}
		printf DAT "SYMNO\t%d\n", $symno;
